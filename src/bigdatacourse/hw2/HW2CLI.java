...
public class HW2CLI {
	...
	public void runCLI() {
		...
		// TODO remove
		try {
			hw2API.connect(pathAstraDBFolder + FILE_ASTRA_DB, getUsername(pathAstraDBFolder + FILE_GENERATED_TOKEN),
					getPasswrod(pathAstraDBFolder + FILE_GENERATED_TOKEN), HW2_KEYSPACE);
		} catch (Exception e1) {
		...
		while (isRunning) {
			...
			try {
				switch (input) {
				case "connect":
					hw2API.connect(pathAstraDBFolder + FILE_ASTRA_DB,
				...
				case "initialize":
					...
					break;
				case "loadItems":
					hw2API.loadItems(pathDatasetFolder + FILE_DATASET_ITEMS);
					break;
				case "loadReviews":
					hw2API.loadReviews(pathDatasetFolder + FILE_DATASET_REVIEWS);
					break;
				case "item":
					hw2API.item(tokens[1]);
					break;
				case "userReviews":
					hw2API.userReviews(tokens[1]);
					break;
				case "itemReviews":
					hw2API.itemReviews(tokens[1]);
					break;
				...
				default:
					System.out.println("Unknonw input. type 'help'");
					break;
				}
			} catch (Exception e) {
	...
	private void printHelp() {
		...
		System.out.println("itemsReviews *\t\t print the reviews for item *");
	...
	private void parsePassedFolders(String[] args) throws Exception {
		if (args.length != 2)
			throw new Exception(
					"ERROR - 2 folder paths are required to be passed: first astradb and then dataset folders");
